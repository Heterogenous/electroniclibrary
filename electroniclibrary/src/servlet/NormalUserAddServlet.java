 ...
 */
@WebServlet("/NormalUserAddServlet")
public class NormalUserAddServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(userService.findM(mail)||userService.findUN(username)) {
				...
				request.getRequestDispatcher("/admin/normaluser-add.jsp").forward(request, response);
			}else {
				//添加用户数据
				User user=new User();
				...
				if(b) {
					request.getSession().setAttribute("adminAdd", "添加用户成功!");
				}else {
					request.getSession().setAttribute("adminAdd", "添加用户失败!");
				}
