 ...
 * 处理管理修改超级用户的操作
 */
@WebServlet("/SuperUserEditServlet")
public class SuperUserEditServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//判断用户是否修改
		if(user.getPassword().equals(password)&&user.getSex().equals(sex)&&user.getMail().equals(mail)) {
			//没有修改则返回到超级用户管理的页面
			...
			if(!user.getMail().equals(mail)) {
				...
				if(a) {
					...
					request.getSession().setAttribute("upSuc", updateSuc);
					response.sendRedirect(request.getContextPath()+"/admin/admin-superuser.jsp");
					return;
				}
			}
